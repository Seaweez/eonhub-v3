...
export const InventoryNftCharacter = ({ data, gameId }: { data: InventoryResponseDTO[], gameId: number }) => {
...
//   const [sealMetaverseAccountInfo, setSealMetaverseAccountInfo] = useAtom(SealMetaverseAccountInfoAtom);
  ...
  useEffect(() => {
    ...
    userService.fetchZeroInfo(userCookie?.token, setCookie, removeCookie, setZeroEternalLoveAccountInfo, selectedServer.gameId,selectedServer.serverID);
  }, [selectedServer])
    ...
    Swal.fire({
      ...
      preConfirm: async (option) => {
        try {
          ...
          if (option == 2) { onSendToRoPcGame(item); }
          return option
        } catch (error) {
  ...
  const onSendToGame = async (item: InventoryResponseDTO) => { 
    userService.fetchZeroInfo(userCookie?.token, setCookie, removeCookie, setZeroEternalLoveAccountInfo, selectedServer.gameId, selectedServer.serverID).then(async (res: any) => {
      ...
      Swal.fire({
        ...
        html: `
          <div id="swal-radio" class="swal2-radio">${gameId === ZERO_ETERNAL_LOVE_GAME_ID ? zeroCharNames.map((char, index) =>
  ...
  const onSendToRoPcGame = async (item: InventoryResponseDTO) => { 
    userService.fetchZeroInfo(userCookie?.token, setCookie, removeCookie, setZeroEternalLoveAccountInfo, selectedServer.gameId, selectedServer.serverID).then(async (res: any) => {
      const zeroCharNames = res?.data?.data?.characterNames || []
  ...
  // white left tab
  return   (<>
    <div className="relative border-2 h-full rounded-2xl bg-neutral-100 bg-opacity-95 mb-10">
      <span className="absolute -top-2 -left-6 bg-secondary text-white px-3 py-1 rounded">Available Characters</span>
      <div className="flex pl-4 pr-6 py-4 ">
        {zeroEternalLoveAccountInfo?.characterNames?.map(char => (<>
          <div className="text-black min-w-40 ">
            <div className="flex justify-center transition-all ">
              <div className="relative  ">
                <div className=" text-white   bg-neutral-800 bg-opacity-95 shadow-md mt-3  rounded-lg text-md  px-4 py-2">
                  {/* <Image alt='' src={`${item.characterName}`} className={`cursor-pointer hover:scale-105 min-h-12 min-w-12`} height={100} width={100} onClick={() => onItemClick(item)} /> */}
                  <div className="bg-primary font-bold  text-center px-8 mb-1">{char.characterName}</div>
                  <div className="pl-1">Lv. {char.level}</div>
                  <div className="pl-1">Zeny: {char.zeny?.toLocaleString('en-US', {})}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="transition-all">
        {zeroEternalLoveAccountInfo?.characterNames?.length > 0 && (
            <div className="cursor-pointer  bg-white rounded-b-xl"
                onClick={()=> toast.error('Transfer in not available!')}
            >
