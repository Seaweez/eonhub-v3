...
export const InventoryNft = ({ data, gameId }: { data: InventoryResponseDTO[], gameId: number }) => {
  ...
  const onItemClick = async (item: InventoryResponseDTO) => {
    ...
    if(!selectedServer?.serverID) return toast.error(`Please select a server !`);
        ...
        preConfirm: () => {
          ...
          return new Promise((resolve, reject) => {
            if (!selectedCharacter) {
              ...
              marketService.addItemToGame(ZERO_ETERNAL_LOVE_GAME_ID, userCookie?.token, item.inventoryId, sendingToChar.characterId, Number(amountToSend), selectedServer.serverID, token)
                .then(res => {
                  marketService.getMyInventory(ZERO_ETERNAL_LOVE_GAME_ID, userCookie?.token)
                    .then(res => {
  ...
  // white left tab
  return data && (<>
    <div className="relative border-2 h-full  rounded-2xl bg-neutral-600 bg-opacity-95 max-h-80 overflow-y-auto">
